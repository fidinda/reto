...
use crate::{encode::{Buffer, Encodable, EncodingError}, parse_tlvs, TLV};
...
enum NameInner<'a> {
    Empty,
    Buffer { component_bytes: &'a [u8],  component_count: usize, original_count: usize },
    Component { original: &'a Name<'a>, component: NameComponent<'a> }
}

impl<'a> Name<'a> {
    pub fn new() -> Self {
        Self{ inner: NameInner::Empty }
    }
    ...
    pub(crate) fn from_bytes(component_bytes: &'a [u8]) -> Option<Self> {
        ...
        let inner = if component_count == 0 {
            ...
            NameInner::Buffer { component_bytes, component_count, original_count: component_count }
        };
    ...
    pub fn component_count(&self) -> usize {
        match self.inner {
            NameInner::Empty => 0,
            NameInner::Buffer { component_count, .. } => component_count ,
    ...
    pub fn dropping_last_component(&self) -> Option<Self> {
        match self.inner {
            ...
            NameInner::Buffer { component_bytes, component_count, original_count } => {
                if component_count == 1 {
                    return Some(Name { inner: NameInner::Empty })
                }
                Some(Name { inner: NameInner::Buffer {
                    component_bytes, 
    ...
    pub fn adding_component(&'a self, component: NameComponent<'a>) -> Self {
        Name { inner: NameInner::Component { original: self, component }}
    }
    ...
    pub fn components(&self) -> impl Iterator<Item = NameComponent<'a>> {
        ...
        self.compute_iter(&mut innermost_bytes, &mut innermost_count, &mut free_components);

        let innermost_iter = innermost_bytes
        .map(|bytes| {
            parse_tlvs(bytes)
            .map(|tlv| {
                ...
                NameComponent { typ, bytes: tlv.val }
            })
            .take(innermost_count)
        });

        NameComponentIterator { innermost_iter, free_components, name: *self }
    }
    ...
    fn compute_iter(
        ...
        match self.inner {
            NameInner::Empty => {},
            NameInner::Buffer { component_bytes, component_count, .. } => {
    ...
    fn component_encode<B: Buffer>(&self, buffer: &mut B) -> Result<(), EncodingError> {
        match self.inner {
            ...
            NameInner::Component { original, component } => {
...
where I : Iterator<Item = NameComponent<'a>> {
            ...
            None => {
                ...
                while depth < self.free_components {
                    ...
                    (nn, cc) = match nn.inner {
                        NameInner::Component { original, component } => (*original, Some(component)),
...
impl<'a> From<NameComponent<'a>> for TLV<'a> {
    fn from(value: NameComponent<'a>) -> Self {
        TLV { typ: value.typ.into(), val: &value.bytes }
    }
}
...
impl From<NameComponentType> for NonZeroU16 {
    fn from(value: NameComponentType) -> Self {
        match value {
            ...
            NameComponentType::ImplicitSha256Digest => NAME_COMPONENT_TYPE_IMPLICIT_SHA256.try_into().unwrap(),
            NameComponentType::ParameterSha256Digest => NAME_COMPONENT_TYPE_PARAMETER_SHA256.try_into().unwrap(),
