...
impl Tables {
        ...
        for entry in self.entries.entries.iter_mut() {
            ...
            for i in (0..entry.pit.pits_len as usize).rev() {
                if entry.pit.reply_to[i] == face {
                    ...
                    entry.pit.can_be_prefix[i] = entry.pit.can_be_prefix[entry.pit.pits_len as usize - 1];
    ...
    pub fn register_prefix<'a>(&mut self, name_prefix: Name<'a>, face: FaceHandle) {
        ...
        let (mut entry, insertion_index) = if name_prefix.component_count() == 0 {
            ...
            let (entry_index, _) = self.entries.get_or_create_nonroot_entry_index_for_name(name_prefix);
            let insertion_index = self.entries.next_insertion_index();
    ...
    pub fn hops_for_name<'a>(&mut self, name: Name<'a>) -> impl Iterator<Item = FaceHandle> +'_ {
        ...
        for (entry_index, depth) in self.entries.entries_mut_for_prefixes_of_name(name) {
            if let Some(entry_index) = entry_index {
                ...
                for i in 0..entry.fibs_len as usize {
                    self.face_map.entry(entry.fibs[i].next_hop)
    ...
    pub fn register_interest<'a>(
        ...
        let (entry_index, is_new) = self.entries.get_or_create_nonroot_entry_index_for_name(name);
        ...
        if is_new || (!entry.is_continued && entry.pit.pits_len == 0) {
            ...
            entry.pit.next_transmission = now.max(entry.pit.next_transmission.adding(retransmission_period));
        ...
        loop {
            for (idx, face) in entry.pit.reply_to[..entry.pit.pits_len as usize].iter().enumerate() {
                if face == &reply_to {
    ...
    pub fn satisfy_interests<'a>(
        ...
        let digest_component = NameComponent::new(
            NameComponentType::ImplicitSha256Digest, &digest
        );
        let full_name = name.adding_component(digest_component);

        // TODO: fix allocations
        let vec : Vec<_> = self.entries.entries_mut_for_prefixes_of_name(full_name).collect();
        ...
        for (entry_index, depth) in vec {
            ...
            for i in (0..entry.pit.pits_len as usize).rev() {
                if entry.pit.can_be_prefix[i] || is_full_name {
                    // Should return face...
                    self.face_map.entry(entry.pit.reply_to[i])
                    .and_modify(|curr| *curr = depth.max(*curr))
                    .or_insert(depth);
                    // ...and remove it
                    entry.pit.reply_to[i] = entry.pit.reply_to[entry.pit.pits_len as usize - 1];
        ...
        now.difference(&self.time_of_last_update).unwrap_or(MS_BEFORE_PRUNE) >= MS_BEFORE_PRUNE 
        {
    ...
    fn prune_stale_entries<'a>(&mut self, deadline: Option<Timestamp>) {
        ...
        let (first_child, _, _) = Self::compact_entry_chain(
            None, 
            &mut self.entries, 
            None, 
            None
    ...
    fn compact_entry_chain(
    ...
    ) -> (Option<EntryIndex>, Option<EntryIndex>, Option<ParentOrSibling>) {
        ...
        loop {
            ...
            if first_child.is_none() { first_child = entry.first_child };
        ...
        if (fib_count + pit_count == 0) && first_child.is_none() {
            ...
            match target {
                Some(ParentOrSibling::Parent(None)) => entries.root_mut().first_child = next_sibling,
                Some(ParentOrSibling::Parent(Some(e))) => entries.entry_mut(&e).first_child = next_sibling,
                Some(ParentOrSibling::Sibling(e)) => entries.entry_mut(&e).next_sibling = next_sibling,
            ...
            let mut next_sibling = match initial {
                Some(idx) => {
                    entries.remove(idx).next_sibling
                },
...
impl<const N : usize, const F : usize, const P : usize> Entries <N, F, P> {
    fn new(first_one: TableEntry<N, F, P>) -> Self {
        Self { entries: alloc::vec![first_one] } // will never be deleted
    }
    ...
    fn next_insertion_index(&self) -> EntryIndex {
        for (idx, ee) in self.entries.iter().enumerate() {
            if ee.is_unused {
                return EntryIndex{ idx: (idx as u32).try_into().unwrap() }
            }
        }
        let idx = self.entries.len();
        EntryIndex{ idx: (idx as u32).try_into().unwrap() }
    }
    ...
    fn get_or_create_nonroot_entry_index_for_name<'a>(&mut self, name: Name<'a>) -> (EntryIndex, bool) {
        ...
        'comp: for component in name.components() {
            ...
            loop {
                ...
                match target {
                    ...
                    ParentOrSibling::Parent(Some(e)) => self.entry_mut(&e).first_child = Some(index),
    ...
    fn entries_mut_for_prefixes_of_name<'a>(&'a self, name: Name<'a>) -> impl Iterator<Item = (Option<EntryIndex>, usize)> + 'a {
        EntryIterator { name_component_iter: name.components(), entries: self, entry_index: None, depth: 0 }
    }

}


struct EntryIterator<'a, I : Iterator<Item = NameComponent<'a>>, const N : usize, const F : usize, const P : usize> {
...
impl<'a, I : Iterator<Item = NameComponent<'a>>, const N : usize, const F : usize, const P : usize> Iterator for EntryIterator<'a, I, N, F, P> {
...
impl Default for FibEntry {
    fn default() -> Self {
        Self { next_hop: FaceHandle(u32::MAX) }
    }
}
...
impl<const P : usize> Default for PitEntry<P> {
    fn default() -> Self {
        Self { 
            ...
            deadline: Timestamp { ms_since_1970: u64::MIN }, 
            nonces: Default::default(), 
            nonces_len: 0,
            next_transmission: Timestamp { ms_since_1970: u64::MAX },
            pits_len: 0, 
        }
    }
}
